...
-(void)doneEditing {
	...
	NSString *strTxtFrom = txtFrom.text;
	NSMutableString *strTxtTo = [NSMutableString stringWithCapacity:txtTo.text.length];
	
	NSScanner *scanner = [NSScanner scannerWithString:txtTo.text];
	NSCharacterSet *numbers = [NSCharacterSet characterSetWithCharactersInString:@"0123456789"];
	
	while ([scanner isAtEnd] == NO) {
		NSString *buffer;
		if ([scanner scanCharactersFromSet:numbers intoString:&buffer]){
			[strTxtTo appendString:buffer];
		} else {
			[scanner setScanLocation:([scanner scanLocation]+1)];
		}
	}
	
	if (textView.text.length > 100 || textView.text.length < 1) {
		UIAlertView *alert;
		alert = [[UIAlertView alloc] 
				 initWithTitle:@"cheatSMS" 
				 message:@"La longitud del mensaje tiene que ser entre 1 y 100 carácteres!" 
				 delegate:self 
				 cancelButtonTitle:@"Aceptar" 
				 otherButtonTitles:nil];
		[alert show];
		[alert release];
	} else if (strTxtFrom.length > 11 || strTxtFrom.length < 1) {
		UIAlertView *alert;
		alert = [[UIAlertView alloc] 
				 initWithTitle:@"cheatSMS" 
				 message:@"La longitud del origen tiene que ser entre 1 y 11 carácteres!" 
				 delegate:self 
				 cancelButtonTitle:@"Aceptar" 
				 otherButtonTitles:nil];
		[alert show];
		[alert release];
	}else if (strTxtTo.length != 11 && strTxtTo.length != 9) {
		UIAlertView *alert;
		alert = [[UIAlertView alloc] 
				 initWithTitle:@"cheatSMS" 
				 message:@"La longitud del destino tiene que ser de 9 o de 11 números!" 
				 delegate:self 
				 cancelButtonTitle:@"Aceptar" 
				 otherButtonTitles:nil];
		[alert show];
		...
		controller.body = [[[[[@"JOKE " stringByAppendingString:strTxtFrom] stringByAppendingString:@" "] 
							 stringByAppendingString:strTxtTo] stringByAppendingString:@" "]
						   stringByAppendingString:textView.text];
	
		controller.recipients = [NSArray arrayWithObjects:@"25532", nil];
...
-(void)messageComposeViewController:(MFMessageComposeViewController *)controller didFinishWithResult:(MessageComposeResult)result{
	...
	switch (result) {
		...
		case MessageComposeResultSent:
			// afegir un missatge no alert dient que ok!
			break;
